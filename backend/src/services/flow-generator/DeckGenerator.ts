...
export class DeckGenerator {
    ...
    for (const node of nodes) {
      ...
      switch (nodeType) {
        ...
        case 'route':
          ...
          break;
        case 'plugin':
          plugins.push(this.generatePlugin(node, nodeData, edges));
          break;
        case 'consumer':
  ...
  private generatePlugin(node: Node, data: any, edges: Edge[]): any {
    const plugin: any = {
      name: data.name || 'rate-limiting',
    ...
    // Find what this plugin is attached to
    const targetEdge = edges.find((e) => e.source === node.id);
    if (targetEdge) {
      // Determine if it's attached to service, route, or consumer
      // This would require looking up the target node type
      // For now, just use the target id as a reference
      plugin._target = targetEdge.target;
    }
